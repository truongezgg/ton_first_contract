...
export function mainContractConfigToCall(config: MainContractConfig) {
  return beginCell()
...
export class MainContract implements Contract {
  ...
  static createFromConfig(
    ...
    const data = mainContractConfigToCall(config);
  ...
  sendNoCodeDeposit(provider: ContractProvider, sender: Sender, value: bigint) {
  ...
  sendDestroy(provider: ContractProvider, sender: Sender, value: bigint) {
    const msg_body = beginCell().storeUint(4, 32).endCell();
    return provider.internal(sender, {
      value,
      sendMode: SendMode.PAY_GAS_SEPARATELY,
      body: msg_body,
